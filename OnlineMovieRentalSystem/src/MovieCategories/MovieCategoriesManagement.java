...
public class MovieCategoriesManagement {
    ...
    public void addMovieCategory() {
        ...
        try {
            PreparedStatement ps = connect.prepareStatement("INSERT INTO "
                    + "MovieCategory(movie_id, category_id) VALUES (?, ?)");
            ps.setInt(1, inputHelper.readInt("Enter Movie ID: "));
            ps.setInt(2, inputHelper.readInt("Enter Category ID: "));

            int count = ps.executeUpdate();
            if (count > 0) {
                System.out.println("Add Movie Category Successfully!!");
                System.out.println("***--*-*-*-*--*-***-*-*-*-*-*-*-*-*--*---****-*-*-*-*-*-*-******--*-*-*--*--*-*-*--*-*-*-***-****-*--*****----*--**");
            } else {
                System.out.println("Add Movie Category failed !!");
            }
    ...
    public void deleteMovieCategory() {
        ...
        do {
            try {
                ...
                PreparedStatement ps = connect.prepareStatement("DELETE FROM MovieCategory WHERE movie_id = ?");
    ...
    public void showMovieCategory() {
        ...
        try {
            ...
            System.out.printf("| %-5s | %-20s | %-30s | %-5s | %-12s | %-10s | %-5s | %-20s |\n",
            ...
            while (rs.next()) {
                found = true;
                System.out.printf("| %-5d | %-20s | %-30s | %-5.1f | %-12s | %-10.2f | %-5d | %-20s |\n",
                        rs.getInt("movie_id"),
                        rs.getString("title"),
